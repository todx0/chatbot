...
import { API_ID, API_HASH, SESSION, recapTextRequest, toxicRecapRequest, BOT_ID, } from './config';
import { writeToHistoryFile, clearHistory, getHistory, qHistory, replHistory } from './app/history/history';
import { sleep, convertToImage, downloadFile, filterMessages, approximateTokenLength, convertFilteredMessagesToString, splitMessageInChunks } from './app/helper';
import { generateGptResponse, createImageFromPrompt, generateGptResponses, combineAnswers } from './app/openai/api';
const client = new TelegramClient(new StringSession(SESSION), +API_ID, API_HASH, {
    connectionRetries: 5,
});
async function sendMessage(obj) {
    const { peer, message, replyToMsgId, silent } = obj;
